...
import { FRAME_HEADER_LENGTH, fnv1a, packFrame, parseFrame } from "./protocol.mjs";
...
export class BeamSender {
  ...
  nextFrame() {
    ...
    return packFrame({ sessionId: this.sessionId, sequence, blockCount: this.encoder.blockCount, blockLength: this.blockLength, totalLength: this.payload.byteLength, payloadFnv: this.payloadFnv }, this.encoder.encode(sequence));
  }

  get stats() {
    return { sessionId: this.sessionId, blockCount: this.encoder.blockCount, blockLength: this.blockLength, totalLength: this.payload.byteLength, sequence: this.sequence };
  }
}

export class BeamReceiver {
  constructor() { this.reset(); }
  reset() { this.decoder = null; this.sessionId = null; this.payloadFnv = null; this.completePayload = null; }
  ...
  add(bytes) {
    ...
    if (header.blockCount !== this.decoder.blockCount || header.blockLength !== this.decoder.blockLength || header.totalLength !== this.decoder.totalLength || header.payloadFnv !== this.payloadFnv) {
      return { accepted: false, reason: "session-parameters-changed" };
    }
    this.decoder.addFrame(header.sequence, block);
    if (this.decoder.isComplete && !this.completePayload) {
      const payload = this.decoder.assemble();
      if (fnv1a(payload) !== this.payloadFnv) { this.reset(); return { accepted: false, reason: "payload-integrity-failed" }; }
